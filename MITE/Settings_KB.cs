...
namespace MITE
{
    public class OptionsKeymapping : UICustomControl
    {
        ...
        //public static SavedInputKey keyStepOver = new SavedInputKey("stepOver", SettingsClass.SettingsFileLocation, SavedInputKey.Encode(KeyCode.Tab, true, false, false), true);

        private int count = 0;

        private void Awake()
        {
            AddKeymapping("Step Over shortcut:", MITE.Settings.keyStepOver.SIK);
        }
        ...
        private void OnBindingKeyDown(UIComponent comp, UIKeyEventParameter p)
        {
            if (this.m_EditingBinding != null && !this.IsModifierKey(p.keycode))
            {
                ...
                MITE.Settings.keyStepOver = new SerializableShortcut(keycode, p.control, p.shift, p.alt);
                MITE.Settings.SaveConfiguration();
            }
        }
